...
class MSDCrossEncoderLayer(nn.Module):
    def __init__(self, config: MSDConfig, layer_idx: int):
        super().__init__()
    ...
    def forward(
        ...
        **kwargs,
        ...
        cross_attention_mask = self._update_cross_attention_mask(
            attention_mask, hidden_states, cache_position, past_key_values, False
        )
    ...
    def _update_cross_attention_mask(
        ...
        # Generate a cross-attention mask (4D) based on the provided attention mask
        cross_attention_mask = self._prepare_4d_cross_attention_mask_with_cache_position(
            attention_mask,
        ...
        if (
            ...
            # Attend to all tokens in fully masked rows in the cross_attention_mask
            cross_attention_mask = AttentionMaskConverter._unmask_unattended(cross_attention_mask, min_dtype)

        return cross_attention_mask
    

    def _prepare_4d_cross_attention_mask_with_cache_position(
        self,
        attention_mask: torch.Tensor,
        sequence_length: int,
        target_length: int,
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            # If the mask is already 4D, we assume it's in the correct format
            return attention_mask
        else:
    ...
    def merge_configs_from_src(self, target_config, src_config):
        ...
        # Iterate through the keys in the Llama config
...
if __name__ == "__main__":
    model = Qwen2VLHL.from_pretrained("Qwen/Qwen2-VL-7B-Instruct", torch_dtype="auto", device_map="cuda")
    print(model)